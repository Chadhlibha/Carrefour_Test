...
class GenererInput(path:String, firstDay:String,nbrOfDays:Int) extends Thread{
  ...
  def genererTransactions(listOfDays: List[String], nbr_transactions:Int,listdesmagasins : List[String]): Unit = {
    ...
    for (day <- listOfDays) {
      ...
      val file = new File(String.format(path+"/data/input/transactions/transactions_%s.data", day.split("T")(0)))
      val bw = new BufferedWriter(new FileWriter(file))
      for (i <- 1 to nbr_transactions) {
        bw.write(i + "|" + day + "|" + listdesmagasins(random.nextInt(listdesmagasins.length)) + "|" + random.nextInt(nbrProduit) + "|" + random.nextInt(qteMax) + "\n")
      }
